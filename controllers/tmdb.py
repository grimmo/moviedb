...
def update_movie_by_id():
    ...
    db(db.film.id==movie_id).update(titolo=risultato['title'],slug=IS_SLUG(check=False)(risultato['title'].encode('utf-8'))[0],anno=strftime('%Y',strptime(risultato['release_date'],u'%Y-%m-%d')),tmdb_id=risultato['id'],trama=risultato['overview'],locandina=floca)    
    ...
    if f:
        ...
        for persona in risultato['cast']:                        
            db.moviecast.update_or_insert(nome=persona['name'],tmdb_id=persona['id'],slug=IS_SLUG(check=False)(persona['name'].encode('utf-8'))[0])       
